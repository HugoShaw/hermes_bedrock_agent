...
class MultiKBClient:
    ...
    ----------
    settings:
        Resolved Settings (may contain one or more KBEntry items).
    kb_ids:
        Optional explicit list of KB IDs to query.  When given, only those
        KBs are queried (they must be present in settings.knowledge_bases).
        Defaults to *all* configured KBs.
    max_workers:
        Thread-pool size for parallel requests (default: number of KBs).
    """
    ...
    def retrieve(
        ...
        ----------
        query:
            The natural-language question / search text.
        number_of_results:
            How many chunks to fetch *per KB* (1-20).
        merge_strategy:
            How to rank the merged list:
            - ``"score"``   — sort by relevance score descending (default)
            - ``"round_robin"`` — interleave one result per KB in turn
            - ``"kb_order"`` — keep KB order, append results sequentially
        deduplicate:
            If True (default), drop chunks with identical text across KBs.

        Returns
        -------
        List[KBResult] sorted according to *merge_strategy*.
        """
        if not query.strip():
            raise ValueError("Query must not be empty.")
        if number_of_results < 1 or number_of_results > 20:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self._max_workers) as pool:
            futures = {
                pool.submit(
                    self._fetch_one, kb, query, number_of_results
                ): kb
                for kb in self._kbs
            }
    ...
    def retrieve_from(
        ...
        """Query a *single* KB by ID (must be one of the configured KBs)."""
    ...
    @staticmethod
    def _merge(
        per_kb: dict[str, list[KBResult]], strategy: str
    ) -> list[KBResult]:
        if strategy == "score":
...
# ---------------------------------------------------------------------------
# Low-level helper (shared by both client classes)
# ---------------------------------------------------------------------------
