...
dualrag project scan s3://your-bucket/プロジェクト名/ -p "project_id"

# ② 全ファイルをマルチタイプ解析（PDF, DOCX, CSV, Image 等）
dualrag project parse-all -p "project_id"
...
4. **`dualrag graph`** — Markdown からエンティティ/関係を抽出し Neptune（グラフDB）にロード（v4.2: 47 エンティティタイプ、55 リレーションシップ）
...
| `--append`, `-a` | 既存プロジェクトデータに追記（以前のワークブックのチャンクを削除しない） | False |
...
# ワークブック 2（--append で既存チャンクを保持したまま追記）
...
> 💡 `--append` なしの場合、同じ `project_id` の既存チャンクが上書きされます。複数ワークブックを順次投入する場合は 2 件目以降に `--append` を付けてください。
...
**Semantic Map v4.2** — プロジェクト非依存の設計で、以下のエンティティ分類体系（7 レイヤー）に基づく抽出を実行します：

| レイヤー | 含まれるエンティティタイプ |
|---------|------------------------|
| Business / Process | BusinessProcess, BusinessObject, BusinessStep, BusinessRule |
| System / Interface | System, ExternalSystem, InternalSystem, Middleware, IntegrationTool, Interface |
| API / Operation | APIOperation, APIEndpoint, APICallSequence, APICallStep |
| Data / Fields | DataEntity, RecordType, Field, FieldDefinition, Parameter, EnumValue |
| Mapping / Rules | MappingDefinition, TransformationRule, ConversionRule, DefaultValueRule, LookupRule, CalculationRule, FilterCondition, Constraint |
| Flow / Implementation | FunctionModule, FlowNode, DecisionPoint, BranchCondition, ScriptStep, Script, FileObject |
| Review / Quality | ReviewTask, Issue, Requirement |
...
```
Phase 0:  Markdown ファイルスキャン（入力ディレクトリ探索）
Phase 1:  エビデンスユニット分割（section / table）
Phase 2:  LLM 2パス抽出（ノード → エッジ、Claude Sonnet 使用）
Phase 3:  正規化・重複排除（ID レジストリ構築）
Phase 4:  構造レイヤー構築（Project → Workbook → Sheet 階層）
Phase 5:  クロスドキュメントリンク生成（名前マッチング）
Phase 6:  レビュータスク生成（低信頼ノード/エッジの検出）
Phase 7:  表示グラフフィルタ（Graph Explorer 用サブセット）
Phase 8:  JSONL 出力保存
Phase 9:  プリフライト検証（P0: 空グラフ、P1: 孤立ノード、P2: 型カバレッジ）
Phase 10: Cypher 生成（Neptune Analytics 互換）
Phase 11: Graph Explore クエリ生成
Phase 12: 抽出レポート生成（検証 Cypher 付き）
Phase 13: Neptune ロード（ノード MERGE → エッジ MERGE）
```

**v4.2 の主な改善点：**

- **プロジェクト非依存プロンプト**: 特定の業種やシステム名に依存しない汎用抽出
- **47 エンティティタイプ対応**: v3.1 の 25 タイプから大幅拡張
- **データレイヤーフィールド**: `field_code`, `data_type`, `length`, `required` 等のフィールド属性を直接抽出
- **55 種類のリレーションシップ**: 関係性ホワイトリストに基づく正確なエッジ生成
- **信頼度ルール**: 0.50 未満の候補は `candidate` スコープに自動分類
- **`link_method` プロパティ**: 各エッジが `explicit_text`, `inferred_layout`, `cross_sheet_name_match` 等の根拠を保持
- **検証 Cypher クエリ**: レポートに Graph Explorer 用の確認クエリを自動付与
- **エンティティ型カバレッジ警告**: 重要タイプ (BusinessProcess, DataEntity, System, BusinessRule) が 0 件の場合に P2 警告
...
```bash
# ① プロジェクトスキャン（マニフェスト生成）
dualrag project scan s3://your-bucket/洋马发动机/ -p "yangma_v2"

# ② スキャン結果を確認
dualrag project status -p "yangma_v2"

# ③ 全ファイルを一括解析
dualrag project parse-all -p "yangma_v2"

# ④ ドライランで分類のみ確認（解析実行なし）
dualrag project parse-all -p "yangma_v2" --dry-run
```
...
> 💡 Excel ファイルは「既存パイプラインで処理済み」としてマーク（`already-handled`）されます。  
> ⚠️ `parse-all` の出力はまだ下流パイプライン (`build-kb`, `graph`) に直接接続されていません。接続は今後対応予定です。
...
│   ├── HULFT开发合同_洋马_.md
│   ├── 洋馬金蝶云星空WEBAPI接口设计说明书V3.0.md
│   ├── YSH_IF_001_mapping_json_001.md
│   └── テスト_SCMCONN_IN.md
...
# マニフェストが生成されていることを確認
cat outputs/yangma_v2/manifest.json | python3 -c "
import json, sys
...
ls outputs/yangma_v2/parsed/ | wc -l

# parsing_manifest.json で解析ステータスを確認
cat outputs/yangma_v2/parsing_manifest.json | python3 -c "
import json, sys
m = json.load(sys.stdin)
stats = {}
...
head -10 outputs/yangma_v2/parsed/*.md | grep -A5 "^---"
```
...
# エンティティタイプ別分布
r = c.execute_query(\"MATCH (n) WHERE n.project_id = 'sample_20260529' RETURN labels(n) AS lbl, count(n) AS cnt ORDER BY cnt DESC LIMIT 10\")
for row in r['results']:
    print(f'  {row[\"lbl\"]}: {row[\"cnt\"]}')
"
```

**Graph Explorer 用検証クエリ（v4.2）:**

```cypher
-- ノードタイプ分布
MATCH (n) WHERE n.project_id = 'sample_20260529'
RETURN labels(n) AS entity_type, count(n) AS cnt ORDER BY cnt DESC

-- 関係タイプ分布
MATCH (a)-[r]->(b) WHERE a.project_id = 'sample_20260529'
RETURN type(r) AS relationship, count(r) AS cnt ORDER BY cnt DESC

-- 孤立ノード（エッジなし）
MATCH (n) WHERE n.project_id = 'sample_20260529' AND NOT (n)--()
RETURN n.id, labels(n), n.name LIMIT 20

-- クロスシートリンク（異なるソースファイル間の関係）
MATCH (a)-[r]->(b)
WHERE a.project_id = 'sample_20260529' AND a.source_file <> b.source_file
RETURN type(r) AS rel, count(r) AS cnt ORDER BY cnt DESC

-- 低信頼エッジ（レビュー候補）
MATCH (a)-[r]->(b)
WHERE a.project_id = 'sample_20260529' AND r.confidence < 0.70
...
| graph (LLM extraction) | Claude Sonnet 2パス抽出 (v4.2) | 43 シートで約 $15〜$25（ノード+エッジ 2パス） |
...
- [ ] `dualrag --help` で 5 コマンドが表示される (parse, build-kb, qa, graph, project)
