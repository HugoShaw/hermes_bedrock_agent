"""Route a query through vector + graph retrieval and assemble context."""
...
from .graph_retriever import fetch_graph_context
...
def answer(
    ...
    cfg = cfg or _default_config
    qa_resp = retrieve(
        query=query, top_k=top_k, include_graph=include_graph,
        cfg=cfg, store_path=store_path, collection=collection,
    )

    evidence_images: list = []
    if include_evidence_images and qa_resp.chunks:
        evidence_images = load_evidence_images(qa_resp.chunks, cfg.project_root)
