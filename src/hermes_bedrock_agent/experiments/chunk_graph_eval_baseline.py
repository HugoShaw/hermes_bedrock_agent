"""Baseline Graph Extraction Experiment.

Runs the same chunk x graph evaluation as chunk_graph_eval.py but uses the
baseline Semantic Map v4.0 prompt instead of v4.3.
...
All outputs go to outputs/experiments/chunk_graph_eval_baseline/ — completely
isolated from the v4.3 experiment.

Usage:
    python -m hermes_bedrock_agent.experiments.chunk_graph_eval_baseline \
        --output-dir outputs/experiments/chunk_graph_eval_baseline
"""
...
# Path to existing v4.3 experiment chunks
...
def _rewrite_chunk_metadata(
    ...
    """Rewrite chunk metadata for the baseline experiment.
    ...
    for chunk in chunks:
        ...
        new_chunk["graph_prompt_version"] = BASELINE_PROMPT_VERSION
        rewritten.append(new_chunk)
    return rewritten


def _write_chunks(chunks: list[dict], output_dir: Path) -> dict:
    """Write chunks.jsonl and chunk_stats.json for the baseline run."""
...
# ── Baseline graph extraction ────────────────────────────────────────────────


def _get_baseline_prompt_snapshot() -> tuple[str, str]:
    """Read the baseline prompt file and compute SHA256."""
    prompt_path = _PROJECT_ROOT / BASELINE_PROMPT_FILE
    if prompt_path.exists():
        ...
        prompt_text = (
            ...
            + BASELINE_SYSTEM_PROMPT
            + "\n\n=== BASELINE NODE EXTRACTION PROMPT ===\n"
            + BASELINE_NODE_EXTRACTION_PROMPT
            + "\n\n=== BASELINE EDGE EXTRACTION PROMPT ===\n"
            + BASELINE_EDGE_EXTRACTION_PROMPT
        )
...
def run_baseline_graph_extraction(
    chunks: list[dict],
    inventory: list[dict],
    experiment_project_id: str,
    graph_run_id: str,
    ...
    """Run graph extraction using baseline prompts.

    Same per-file-group approach as v4.3 but with baseline prompt philosophy.
    """
    ...
    prompt_text, prompt_sha = _get_baseline_prompt_snapshot()
    prompt_config = {
        "file_path": BASELINE_PROMPT_FILE,
        ...
        "region": region,
        "max_tokens": 16000,
        "adapter_module": "hermes_bedrock_agent.experiments.baseline_prompts",
    }
    ...
    logger.info(
        "Starting BASELINE graph extraction: %d files, model=%s",
        total_files,
        model_id,
    )
    ...
    for file_idx, (file_path, file_chunk_list) in enumerate(file_chunks.items()):
        ...
        # Pass 1: Node extraction with baseline prompt
        node_prompt = BASELINE_NODE_EXTRACTION_PROMPT.format(
        ...
        try:
            ...
            logger.info(
                "[%d/%d] %s/%s: %d nodes extracted (baseline)",
        ...
        # Pass 2: Edge extraction with baseline prompt
        edges: list[dict] = []
        if nodes:
            node_id_list = "\n".join(
            ...
            edge_prompt = BASELINE_EDGE_EXTRACTION_PROMPT.format(
                project_name=project_display,
                project_id=source_project_key,
                workbook_name=workbook_name,
                sheet_name=sheet_name,
            ...
            try:
                ...
                logger.info(
                    "[%d/%d] %s/%s: %d edges extracted (baseline)",
        ...
        for node in nodes:
            ...
            node["graph_prompt_version"] = BASELINE_PROMPT_VERSION
        ...
        for edge in edges:
            ...
            edge["graph_prompt_version"] = BASELINE_PROMPT_VERSION
    ...
    graph_stats = {
        ...
        "graph_prompt_version": BASELINE_PROMPT_VERSION,
        "model_id": model_id,
    ...
    (qa_dir / "notes.md").write_text(
        ...
        f"Prompt version: baseline (Semantic Map v4.0)\n\n"
    ...
    logger.info(
        "Baseline graph extraction complete: %d nodes, %d edges from %d files",
...
def _write_run_report(
    ...
    report = f"""# Experiment Run Report — Baseline
...
- Graph Prompt Version: `{BASELINE_PROMPT_VERSION}` (Semantic Map v4.0)
- Prompt File: `{BASELINE_PROMPT_FILE}`
...
def generate_comparison(output_dir: Path, experiments: list[dict]) -> None:
    """Generate comparison CSVs and summary across all baseline experiments."""
    ...
    summary = "# Baseline Experiment Summary\n\n"
    summary += f"Generated: {datetime.now(timezone.utc).isoformat()}\n\n"
    summary += "## Prompt Version\n\n"
    summary += f"- **baseline** — Semantic Map v4.0 (`{BASELINE_PROMPT_FILE}`)\n"
    summary += "- Adapted for per-file-group extraction via `baseline_prompts.py`\n\n"
    ...
    summary += "- Richer entity taxonomy (50+ types vs ~35 in v4.3)\n"
    summary += "- Strict relationship whitelist (~60 types)\n"
    summary += "- Explicit confidence upper bounds\n"
    summary += "- Mandatory evidence_text (non-empty)\n"
    summary += "- Extended node schema (view_scope, flow_node_kind, parent_function_id, etc.)\n"
    summary += "- Field Definition vs FieldMapping P0 separation\n"
    summary += "- No verified cross-document links on name similarity alone\n"
    summary += "- Adaptive project pattern detection\n"
...
def main(
    ...
    """Run the baseline experiment pipeline."""
    output_dir = output_dir or (
        _PROJECT_ROOT / "outputs" / "experiments" / "chunk_graph_eval_baseline"
    )
    ...
    logger.info("=" * 60)
    logger.info("BASELINE GRAPH EXTRACTION EXPERIMENT")
    logger.info("=" * 60)
    logger.info("Output dir: %s", output_dir)
    logger.info("Projects: %s", project_keys)
    logger.info("Chunking strategies: %s", strategies)
    logger.info("Graph prompt version: %s", BASELINE_PROMPT_VERSION)
    ...
    for project_key in project_keys:
        ...
        for strategy in strategies:
            ...
            experiment_project_id = (
                f"exp_{project_key}__chunk_{strategy}__graph_{BASELINE_PROMPT_VERSION}"
            )
            chunk_run_id = f"{project_key}__chunk_{strategy}"
            graph_run_id = (
                f"{project_key}__chunk_{strategy}__graph_{BASELINE_PROMPT_VERSION}"
            )
            ...
            if not skip_graph:
                try:
                    all_nodes, all_edges = run_baseline_graph_extraction(
                        chunks=chunks,
                        inventory=inventory,
                        experiment_project_id=experiment_project_id,
                        graph_run_id=graph_run_id,
...
def cli() -> None:
    parser = argparse.ArgumentParser(
        description="Baseline Graph Extraction Experiment (Semantic Map v4.0)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=_PROJECT_ROOT / "outputs" / "experiments" / "chunk_graph_eval_baseline",
